...
export function EditRuleDialog({ open, onOpenChange, rule, onSuccess }: EditRuleDialogProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    timeIntervalMinutes: rule.timeIntervalMinutes?.toString() || '',
    timeIntervalHours: rule.timeIntervalHours?.toString() || '',
    timeIntervalDays: rule.timeIntervalDays?.toString() || '',
    maxMessagesPerDay: rule.maxMessagesPerDay.toString(),
    activeHoursStart: rule.activeHoursStart.toString(),
  ...
  const [includeTags, setIncludeTags] = useState<string[]>(rule.includeTags);
  const [excludeTags, setExcludeTags] = useState<string[]>(rule.excludeTags);
  ...
  useEffect(() => {
    ...
    setIncludeTags(rule.includeTags);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/ai-automations/${rule.id}`, {
        method: 'PATCH',
      ...
      toast.success('AI automation rule updated successfully!');
      onSuccess();
      onOpenChange(false);
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit AI Automation Rule</DialogTitle>
          <DialogDescription>
            Update your automated follow-up rule settings
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Activate or pause this automation
                </p>
              </div>
              <Switch
