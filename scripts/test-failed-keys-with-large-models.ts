/**
 * Test Failed Keys with Large Available Models
 * 
 * Tests the 19 failed keys with large models that are as smart as gpt-oss-120b
 */
...
// Large models comparable to gpt-oss-120b (from discovery)
const LARGE_MODELS = [
  'meta/llama-3.1-405b-instruct',      // 405B - Even larger than 120B!
  'meta/llama-3.1-70b-instruct',       // 70B - Large and capable
  'meta/llama-3.3-70b-instruct',       // 70B - Latest version
  'google/gemma-2-27b-it',             // 27B - Medium-large
];
...
async function testKeyWithModel(key: string, model: string, keyIndex: number): Promise<TestResult> {
...
async function testFailedKeysWithLargeModels() {
  console.log('\n🧪 Testing Failed Keys with Large Available Models\n');
  console.log(`📋 Testing ${FAILED_KEYS.length} keys with ${LARGE_MODELS.length} large models\n`);
  console.log('🎯 Models tested (comparable to gpt-oss-120b):');
  LARGE_MODELS.forEach((m, i) => {
    const size = m.includes('405b') ? '405B' : m.includes('70b') ? '70B' : m.includes('27b') ? '27B' : '?';
    console.log(`   ${i + 1}. ${m} (${size} parameters)`);
  });
  ...
  for (let keyIndex = 0; keyIndex < FAILED_KEYS.length; keyIndex++) {
    ...
    for (let modelIndex = 0; modelIndex < LARGE_MODELS.length; modelIndex++) {
      const model = LARGE_MODELS[modelIndex];
      const modelName = model.split('/').pop() || model;
      process.stdout.write(`   [${modelIndex + 1}/${LARGE_MODELS.length}] ${modelName}... `);
      ...
      if (modelIndex < LARGE_MODELS.length - 1) {
  ...
  // Print summary
  console.log('\n' + '='.repeat(70));
  console.log('📊 LARGE MODELS TEST SUMMARY\n');
  ...
  if (byModel.size > 0) {
    ...
    byModel.forEach((keys, model) => {
      const size = model.includes('405b') ? '405B' : model.includes('70b') ? '70B' : model.includes('27b') ? '27B' : '?';
      console.log(`   📦 ${model} (${size} parameters)`);
      console.log(`      ${keys.length} keys work:`);
      keys.forEach((k, i) => {
    ...
    console.log('❌ No working combinations found with large models\n');
  ...
  const resultsJson = {
    ...
    largeModelsTested: LARGE_MODELS,
  ...
  await fs.writeFile(
    'api-keys-large-models-results.json',
    JSON.stringify(resultsJson, null, 2)
  );

  console.log('💾 Results saved to: api-keys-large-models-results.json\n');
...
testFailedKeysWithLargeModels()
  .then(({ working, byModel }) => {
    ...
    if (working.length > 0) {
      console.log('🎉 Great! Found large models that work!');
      console.log('   Use the code above to add working keys and update your model.\n');
    } else {
      console.log('⚠️  No keys work with large models.');
      console.log('   You may need to request access in your NVIDIA developer account.\n');
    }
